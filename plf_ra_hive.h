...
namespace plf
{
    struct hive_limits // for use in block_capacity setting/getting functions and constructors
    {
        ...
        constexpr hive_limits( size_t const minimum, size_t const maximum ) noexcept
            : min( minimum )
            , max( maximum ) { }
    };
    ...
    {
        using skipfield_type =
        std::conditional_t<( sizeof( element_type ) > 10 || alignof( element_type ) > 10 ), uint_least16_t, uint_least8_t>;
    ...
    private:
        ...
        static auto get_aligned_block_capacity( skipfield_type const elements_per_group ) noexcept -> size_type
        {
            return ( ( elements_per_group * ( sizeof( aligned_element_struct ) + sizeof( skipfield_type ) ) ) + sizeof(
                         skipfield_type ) + sizeof( aligned_allocation_struct ) - 1 ) / sizeof( aligned_allocation_struct );
        }
        ...
        using aligned_pointer_type = aligned_element_struct*;
        // pointer to the (potentially overaligned) element type, not the original element type
        using skipfield_pointer_type = skipfield_type*;

        using aligned_allocator_type = typename std::allocator_traits<allocator_type>::template rebind_alloc<
            aligned_element_struct>;
        using group_allocator_type = typename std::allocator_traits<allocator_type>::template rebind_alloc<group>;
        using skipfield_allocator_type = typename std::allocator_traits<allocator_type>::template rebind_alloc<skipfield_type>;
        using aligned_struct_allocator_type = typename std::allocator_traits<allocator_type>::template rebind_alloc<
            aligned_allocation_struct>;
        using tuple_allocator_type = typename std::allocator_traits<allocator_type>::template rebind_alloc<item_index_tuple>;

        using group_pointer_type          = typename std::allocator_traits<group_allocator_type>::pointer;
        using aligned_struct_pointer_type = typename std::allocator_traits<aligned_struct_allocator_type>::pointer;
        using tuple_pointer_type          = typename std::allocator_traits<tuple_allocator_type>::pointer;
        ...
        static constexpr auto pointer_cast( source_pointer_type const source_pointer ) noexcept -> destination_pointer_type
        {
        ...
        static constexpr auto to_aligned_pointer( source_pointer_type const source_pointer ) noexcept -> aligned_pointer_type
        {
        ...
        struct group
        {
            ...
            // Next group in the linked list of all groups. nullptr if no following group. 2nd in struct because it is so frequently used during iteration.
            aligned_struct_pointer_type const elements; // Element storage.
            ...
            // The index of the last erased element in the group. The last erased element will, in turn, contain the number of the index of the next erased element, and so on. If this is == maximum skipfield_type value then free_list is empty ie. no erasures have occurred in the group (or if they have, the erased locations have subsequently been reused via insert/emplace/assign).
            skipfield_type const capacity;
            // The element capacity of this particular group - can also be calculated from reinterpret_cast<aligned_pointer_type>(group->skipfield) - group->elements, however this space is effectively free due to struct padding and the sizeof(skipfield_type), and calculating it once is faster in benchmarking.
            skipfield_type size;
            // The total number of active elements in group - changes with insert and erase commands - used to check for empty group in erase function, as an indication to remove the group. Also used in combination with capacity to check if group is full, which is used in the next/previous/advance/distance overloads, and range-erase.
            group_pointer_type erasures_list_next_group, erasures_list_previous_group;
            // The next and previous groups in the list of groups with erasures ie. with active erased-element free lists. nullptr if no next or previous group.
            ...
            group(
                ...
                , elements(
                    std::allocator_traits<aligned_struct_allocator_type>::allocate(
                        aligned_struct_allocator, get_aligned_block_capacity( elements_per_group ),
                ...
                , group_number( ( previous == nullptr ) ? 0 : previous->group_number + 1u )
            {
                ...
                std::memset(
                    ...
                    sizeof( skipfield_type ) * ( static_cast<size_type>( elements_per_group ) + 1u ) );
            }


            auto reset(
                skipfield_type const increment, group_pointer_type const next, group_pointer_type const previous,
                size_type const group_num ) noexcept -> void
            {
                next_group                   = next;
                free_list_head               = std::numeric_limits<skipfield_type>::max( );
                previous_group               = previous;
                ...
                std::memset(
                    static_cast<void*>( std::to_address( skipfield ) ), 0,
                    sizeof( skipfield_type ) * static_cast<size_type>( capacity ) );
                // capacity + 1 is not necessary here as the final skipfield node is never written to after initialization
            }
        };
        ...
        group_pointer_type erasure_groups_head,
                           // Head of doubly-linked list of groups which have erased-element memory locations available for re-use
                           unused_groups_head;
        // Head of singly-linked list of reserved groups retained by erase()/clear() or created by reserve()
        ...
        static constexpr auto block_capacity_default_min( ) noexcept -> skipfield_type
        {
            skipfield_type const adaptive_size = static_cast<skipfield_type>(
                ( ( sizeof( hive ) + sizeof( group ) ) * 2 ) / sizeof( aligned_element_struct ) );
            skipfield_type const max_block_capacity = block_capacity_default_max( );
            // Necessary to check against in situations with > 64bit pointer sizes and small sizeof(T)
        ...
        static constexpr auto block_capacity_default_max( ) noexcept -> skipfield_type
        {
            return static_cast<skipfield_type>( std::min(
                std::min( static_cast<size_t>( std::numeric_limits<skipfield_type>::max( ) ), static_cast<size_t>( 8192u ) ),
                max_size_static( ) ) );
        }


        auto check_capacities_conformance( hive_limits const capacities ) const -> void
        {
        ...
        auto blank( ) noexcept -> void
        {
            if constexpr ( std::is_standard_layout<hive>::value && std::allocator_traits<allocator_type>::is_always_equal::value
                           && std::is_trivially_destructible<group_pointer_type>::value )
            {
        ...
        auto reserve_and_range_fill( size_type const size, iterator_type const it ) -> void
        {
            if ( size != 0 )
            {
                reserve( size );
    ...
    public:
        static constexpr auto block_capacity_default_limits( ) noexcept -> hive_limits
        {
            return hive_limits(
                static_cast<size_t>( block_capacity_default_min( ) ), static_cast<size_t>( block_capacity_default_max( ) ) );
        }


        // Default constructors:

        constexpr explicit hive( allocator_type const& alloc ) noexcept
        ...
        constexpr hive( ) noexcept(noexcept(allocator_type( ))) : hive( allocator_type( ) ) { }


        constexpr hive( hive_limits const block_limits, allocator_type const& alloc )
            : allocator_type( alloc )
            , erasure_groups_head( nullptr )
            , unused_groups_head( nullptr )
            , total_size( 0 )
        ...
        constexpr explicit hive( hive_limits const block_limits ) : hive( block_limits, allocator_type( ) ) { }


        // Copy constructors:

        hive( hive const& source, std::type_identity_t<allocator_type> const& alloc )
            : allocator_type( alloc )
            , erasure_groups_head( nullptr )
            , unused_groups_head( nullptr )
            , total_size( 0 )
            ...
            , min_block_capacity(
                std::max(
                    source.min_block_capacity, static_cast<skipfield_type>( std::min(
                        source.total_size, static_cast<size_type>( source.max_block_capacity ) ) ) ) )
            , // min group size is set to value closest to total number of elements in source hive, in order to not create unnecessary small groups in the range-insert below, then reverts to the original min group size afterwards. This effectively saves a call to reserve.
        ...
        hive(
            size_type const fill_number, element_type const& element, hive_limits const block_limits,
        ...
        hive( size_type const fill_number, element_type const& element, allocator_type const& alloc = allocator_type( ) )
            : hive( fill_number, element, block_capacity_default_limits( ), alloc ) { }


        // Default-value fill constructors:

        hive( size_type const fill_number, hive_limits const block_limits, allocator_type const& alloc = allocator_type( ) )
            : hive( fill_number, element_type( ), block_limits, alloc ) { }


        hive( size_type const fill_number, allocator_type const& alloc = allocator_type( ) )
        ...
        hive(
            typename std::enable_if_t<!std::numeric_limits<iterator_type>::is_integer, iterator_type> const& first,
            iterator_type const& last, hive_limits const block_limits, allocator_type const& alloc = allocator_type( ) )
            : allocator_type( alloc )
            , erasure_groups_head( nullptr )
            , unused_groups_head( nullptr )
            , total_size( 0 )
        ...
        hive(
            typename std::enable_if_t<!std::numeric_limits<iterator_type>::is_integer, iterator_type> const& first,
            iterator_type const& last, allocator_type const& alloc = allocator_type( ) )
        ...
        hive(
            std::initializer_list<element_type> const& element_list, hive_limits const block_limits,
            allocator_type const& alloc = allocator_type( ) )
            : allocator_type( alloc )
            , erasure_groups_head( nullptr )
            , unused_groups_head( nullptr )
        ...
        hive( std::initializer_list<element_type> const& element_list, allocator_type const& alloc = allocator_type( ) )
        ...
        hive(
            plf::ranges::from_range_t, range_type&& rg, hive_limits const block_limits,
            allocator_type const& alloc = allocator_type( ) )
            : allocator_type( alloc )
            , erasure_groups_head( nullptr )
            , unused_groups_head( nullptr )
        ...
        template <class range_type>
            requires std::ranges::range<range_type>
        hive( plf::ranges::from_range_t, range_type&& rg, allocator_type const& alloc = allocator_type( ) )
        ...
        auto rbegin( ) noexcept -> reverse_iterator
        {
            return ( end_iterator.group_pointer != nullptr )
                       ? ++reverse_iterator(
                           end_iterator.group_pointer, end_iterator.element_pointer, end_iterator.skipfield_pointer )
                       : reverse_iterator(
        ...
        auto rend( ) noexcept -> reverse_iterator
        {
            return reverse_iterator(
                begin_iterator.group_pointer, begin_iterator.element_pointer - 1, begin_iterator.skipfield_pointer - 1 );
        }
    ...
    private:
        auto allocate_new_group(
            skipfield_type const elements_per_group, group_pointer_type const previous = nullptr ) -> group_pointer_type
        {
            if ( max_size( ) - total_capacity < elements_per_group )
            // Just in case max_size is a lower amount than the actual memory available (uncommon platform). Comparison avoids overflow.
            {
#ifdef PLF_EXCEPTIONS_SUPPORT
            ...
            group_pointer_type const new_group = std::allocator_traits<group_allocator_type>::allocate(
                group_allocator, 1, previous );
            total_capacity += elements_per_group;
            // I don't know why GCC creates better/smaller codegen when this is placed here rather than at bottom of function, But it does.
        ...
        auto deallocate_group( group_pointer_type const the_group ) noexcept -> void
        {
            std::allocator_traits<aligned_struct_allocator_type>::deallocate(
                aligned_struct_allocator, the_group->elements, get_aligned_block_capacity( the_group->capacity ) );
            std::allocator_traits<group_allocator_type>::deallocate( group_allocator, the_group, 1 );
        }


        auto deallocate_group_remove_capacity( group_pointer_type const the_group ) noexcept -> void
        {
        ...
        constexpr auto destroy_element( aligned_pointer_type const element ) noexcept -> void
        {
        ...
        auto destroy_group( aligned_pointer_type const end_pointer ) noexcept -> void
        {
        ...
        auto destroy_all_data( ) noexcept -> void
        {
            if ( begin_iterator.group_pointer != nullptr )
            {
                ...
                if constexpr ( !std::is_trivially_destructible<element_type>::value )
                {
                    if ( total_size != 0 )
                    {
                        while ( begin_iterator.group_pointer != end_iterator.group_pointer )
                        // Erase elements without bothering to update skipfield - much faster:
                        {
                            group_pointer_type const next_group = begin_iterator.group_pointer->next_group;
                            ...
                            begin_iterator.element_pointer =
                                    to_aligned_pointer( next_group->elements ) + *( next_group->skipfield );
        ...
        auto initialize( skipfield_type const first_group_size ) -> void
        {
            end_iterator.group_pointer   = begin_iterator.group_pointer   = allocate_new_group( first_group_size );
            end_iterator.element_pointer = begin_iterator.element_pointer = to_aligned_pointer(
        ...
        auto edit_free_list( skipfield_pointer_type const location, skipfield_type const value ) noexcept -> void
        {
        ...
        auto edit_free_list_prev(
            aligned_pointer_type const location, skipfield_type const value ) noexcept -> void
            // Write to the 'previous erased element' index in the erased element memory location
        {
            edit_free_list( pointer_cast<skipfield_pointer_type>( location ), value );
        }


        auto edit_free_list_next(
            aligned_pointer_type const location, skipfield_type const value ) noexcept -> void // Ditto 'next'
        {
            edit_free_list( pointer_cast<skipfield_pointer_type>( location ) + 1, value );
        }


        auto edit_free_list_head( aligned_pointer_type const location, skipfield_type const value ) noexcept -> void
        {
        ...
        auto update_skipblock( iterator const& new_location, skipfield_type const prev_free_list_index ) noexcept -> void
        {
            skipfield_type const new_value = static_cast<skipfield_type>( *( new_location.skipfield_pointer ) - 1 );
            ...
            if ( new_value != 0 ) // ie. skipfield was not originally length 1, hence we need to truncate it
            {
                ...
                *( new_location.skipfield_pointer + new_value ) = *( new_location.skipfield_pointer + 1 ) = new_value;
                ...
                {
                    edit_free_list_next(
                        to_aligned_pointer( new_location.group_pointer->elements ) + prev_free_list_index,
                        erasure_groups_head->free_list_head );
                }
            ...
            else // single-node skipblock, remove skipblock
            {
                ...
                else // remove this group from the list of groups with erasures
                {
                    ...
                    // No need to update previous group for new head, as this is never accessed if group == head
                }
            }
            ...
            *( new_location.skipfield_pointer ) = 0;
            ...
            if ( new_location.group_pointer == begin_iterator.group_pointer && new_location.element_pointer < begin_iterator.
                 element_pointer )
            {
        ...
        auto update_subsequent_group_numbers( size_type current_group_number, group_pointer_type update_group ) noexcept -> void
        {
            do
            {
        ...
        auto reset_group_numbers( ) noexcept -> void
        {
            update_subsequent_group_numbers( 0, begin_iterator.group_pointer );
        }


        auto reset_group_numbers_if_necessary( ) noexcept -> void
        {
        ...
        auto reuse_unused_group( ) noexcept -> group_pointer_type
        {
        ...
        constexpr auto construct_element( pointer_type const location, arguments&&... parameters ) -> void
        {
            std::allocator_traits<allocator_type>::construct(
                *this, pointer_cast<pointer>( location ), std::forward<arguments>( parameters )... );
        }

    public:
        auto insert(
            element_type const& element ) -> iterator
            // Note: defining insert & and insert && as calls to emplace results in larger codegen in release mode (under GCC at least), and prevents more accurate is_nothrow tests
        {
            if ( end_iterator.element_pointer != nullptr ) // ie. empty hive, no blocks allocated yet
            {
                ...
                {
                    if ( end_iterator.element_pointer != pointer_cast<aligned_pointer_type>(
                             end_iterator.group_pointer->skipfield ) ) // ie. end_iterator is not at end of block
                    {
                        construct_element( end_iterator.element_pointer, element );

                        iterator const return_iterator = end_iterator;
                    ...
                    if ( unused_groups_head == nullptr )
                    {
                        ...
                        next_group = allocate_new_group(
                            static_cast<skipfield_type>( std::min( total_size, static_cast<size_type>( max_block_capacity ) ) ),
                ...
                else // there are erased elements, reuse those memory locations
                {
                    iterator new_location(
                        erasure_groups_head,
                        to_aligned_pointer( erasure_groups_head->elements ) + erasure_groups_head->free_list_head,
                        erasure_groups_head->skipfield + erasure_groups_head->free_list_head );
                    ...
                    skipfield_type const prev_free_list_index = *pointer_cast<skipfield_pointer_type>(
        ...
        auto insert(
            [[maybe_unused]] const_iterator hint, element_type const& element ) -> iterator
            // Note: hint is ignored, purely to serve other standard library functions like insert_iterator
        {
        ...
        auto insert( [[maybe_unused]] const_iterator hint, element_type&& element ) -> iterator
        {
        ...
        auto emplace(
            arguments&&... parameters ) -> iterator
            // The emplace function is near-identical to the regular insert function, with the exception of the element construction method, and change to is_nothrow tests.
        {
            if ( end_iterator.element_pointer != nullptr )
            {
                if ( erasure_groups_head == nullptr )
        ...
        template <typename... arguments>
        auto emplace_hint( [[maybe_unused]] const_iterator hint, arguments&&... parameters ) -> iterator
        {
            return emplace( std::forward<arguments>( parameters )... );
        }


        auto at( size_type index ) const -> element_type&
        {
        ...
        auto ensure_get( size_type index ) const -> element_type*
        {
            assert( index < total_capacity );

            // 1. find correct group
            group_pointer_type current_group = begin_iterator.group_pointer;
            while ( index >= current_group->capacity )
            {
                index -= current_group->capacity;
                current_group = current_group->next_group;
            }

            // 2a. no erasures - direct access
            if ( current_group->free_list_head == std::numeric_limits<skipfield_type>::max( ) )
            {
                return pointer_cast<pointer>( to_aligned_pointer( current_group->elements ) + index );
            }

            // 2b. has erasures - must traverse skipfield
            aligned_pointer_type element_ptr = to_aligned_pointer( current_group->elements );
            skipfield_pointer_type skip_ptr  = current_group->skipfield;

            size_t skip = *skip_ptr;
            std::advance( element_ptr, skip );
            std::advance( skip_ptr, skip );

            for ( size_type i = 0; i < index; ++i )
            {
                std::advance( skip_ptr, *std::next( skip_ptr ) + 1U );

                skip = *skip_ptr;
                std::advance( element_ptr, skip + 1U );
                std::advance( skip_ptr, skip + 1U );
                i += skip;
            }

            return skip == 0U ? pointer_cast<pointer>( element_ptr ) : nullptr;
        }

    private:
        // For catch blocks in fill() and range_fill()
        auto recover_from_partial_fill( ) -> void
        {
#ifdef PLF_EXCEPTIONS_SUPPORT
            if constexpr ( ( !std::is_copy_constructible<element_type>::value && !std::is_nothrow_move_constructible<
                                 element_type>::value ) || !std::is_nothrow_copy_constructible<element_type>::value )
            // to avoid unnecessary codegen, since this function will never be called if this line isn't true
            {
                skipfield_type const elements_constructed_before_exception = static_cast<skipfield_type>(
        ...
        auto fill( element_type const& element, skipfield_type const size ) -> void
        {
#ifdef PLF_EXCEPTIONS_SUPPORT
            if constexpr ( !std::is_nothrow_copy_constructible<element_type>::value )
            {
                aligned_pointer_type const fill_end = end_iterator.element_pointer + size;
...
#endif
                if constexpr ( std::is_trivially_copyable<element_type>::value && std::is_trivially_copy_constructible<
                                   element_type>::value )
                ...
                // If at least nothrow_constructible (or exceptions disabled), can remove the large block of 'catch' code above
                {
                    aligned_pointer_type const fill_end = end_iterator.element_pointer + size;
        ...
        auto recover_from_partial_skipblock_fill(
            aligned_pointer_type const location, aligned_pointer_type const current_location,
            skipfield_pointer_type const skipfield_pointer, skipfield_type const prev_free_list_node ) -> void
        {
#ifdef PLF_EXCEPTIONS_SUPPORT
            if constexpr ( ( !std::is_copy_constructible<element_type>::value && !std::is_nothrow_move_constructible<
                                 element_type>::value ) || !std::is_nothrow_copy_constructible<element_type>::value )
            // to avoid unnecessary codegen
            {
                ...
                skipfield_type const new_skipblock_head_index = static_cast<skipfield_type>( ( location - to_aligned_pointer(
                    erasure_groups_head->elements ) ) + elements_constructed_before_exception );
                erasure_groups_head->free_list_head = new_skipblock_head_index;

                if ( prev_free_list_node != std::numeric_limits<skipfield_type>::max( ) )
                {
                    edit_free_list_next(
                        to_aligned_pointer( erasure_groups_head->elements ) + prev_free_list_node, new_skipblock_head_index );
                }
            }
#endif
        }
        ...
        auto fill_skipblock(
            element_type const& element, aligned_pointer_type const location, skipfield_pointer_type const skipfield_pointer,
            skipfield_type const size ) -> void
        {
#ifdef PLF_EXCEPTIONS_SUPPORT
            if constexpr ( !std::is_nothrow_copy_constructible<element_type>::value )
            {
                aligned_pointer_type const fill_end      = location + size;
                skipfield_type const prev_free_list_node = *pointer_cast<skipfield_pointer_type>( location );
        ...
        auto fill_unused_groups(
            size_type size, element_type const& element, size_type group_number, group_pointer_type previous_group,
            group_pointer_type const current_group ) -> void
        {
            for ( end_iterator.group_pointer = current_group; end_iterator.group_pointer->capacity<size;
                  end_iterator.group_pointer = end_iterator.group_pointer->next_group )
            {
                skipfield_type const capacity = end_iterator.group_pointer->capacity;
    ...
    public:
        ...
        auto insert( size_type size, element_type const& element ) -> void
        {
            if ( size == 0 )
            {
                return;
            ...
            {
                aligned_pointer_type const element_pointer =
                        to_aligned_pointer( erasure_groups_head->elements ) + erasure_groups_head->free_list_head;
                skipfield_pointer_type const skipfield_pointer =
                        erasure_groups_head->skipfield + erasure_groups_head->free_list_head;
                skipfield_type const skipblock_size = *skipfield_pointer;
                ...
                if ( skipblock_size <= size )
                {
                    ...
                    // ie. there are more skipblocks to be filled in this group
                    {
                        edit_free_list_next(
                            to_aligned_pointer( erasure_groups_head->elements ) + erasure_groups_head->free_list_head,
                            std::numeric_limits<skipfield_type>::max( ) );
                ...
                {
                    skipfield_type const prev_index = *pointer_cast<skipfield_pointer_type>( element_pointer );
                    ...
                    skipfield_type const new_skipblock_size = static_cast<skipfield_type>( skipblock_size - size );
                    ...
                    *( skipfield_pointer + skipblock_size - 1 ) = new_skipblock_size;
                    erasure_groups_head->free_list_head         = static_cast<skipfield_type>(
                    ...
                    {
                        edit_free_list_next(
                            to_aligned_pointer( erasure_groups_head->elements ) + prev_index,
                            erasure_groups_head->free_list_head );
            ...
            skipfield_type const group_remainder = static_cast<skipfield_type>( std::min(
                static_cast<size_type>( pointer_cast<aligned_pointer_type>( end_iterator.group_pointer->skipfield ) - end_iterator
                                        .element_pointer ), size ) );
            ...
            // Use unused groups:
            end_iterator.group_pointer->next_group = unused_groups_head;
            if ( ( std::numeric_limits<size_type>::max( ) - end_iterator.group_pointer->group_number ) < size ) [[unlikely]]
                    reset_group_numbers( );
            fill_unused_groups(
    ...
    private:
        template <class iterator_type>
        auto range_fill( iterator_type& it, skipfield_type const size ) -> void
        {
            aligned_pointer_type const fill_end = end_iterator.element_pointer + size;
        ...
        template <class iterator_type>
        auto range_fill_skipblock(
            iterator_type& it, aligned_pointer_type const location, skipfield_pointer_type const skipfield_pointer,
            skipfield_type const size ) -> void
        {
            aligned_pointer_type const fill_end = location + size;
        ...
        template <class iterator_type>
        auto range_fill_unused_groups(
            size_type size, iterator_type it, size_type group_number, group_pointer_type previous_group,
            group_pointer_type const current_group ) -> void
        {
            for ( end_iterator.group_pointer = current_group; end_iterator.group_pointer->capacity<size;
                  end_iterator.group_pointer = end_iterator.group_pointer->next_group )
        ...
        template <class iterator_type>
        auto range_insert(
            iterator_type it, size_type size ) -> void
            // this is near-identical to the fill insert, with the only alteration being incrementing an iterator for construction, rather than using a const element. And the fill etc function calls are changed to range_fill to match this pattern. See fill insert for code explanations
        {
            if ( size == 0 )
            {
                return;
    ...
    public:
        ...
        template <class iterator_type>
        auto insert( std::move_iterator<iterator_type> const first, std::move_iterator<iterator_type> const last ) -> void
        {
        ...
        auto insert( std::initializer_list<element_type> const& element_list ) -> void
        {
        ...
        template <class range_type>
            requires std::ranges::range<range_type>
        auto insert_range( range_type&& the_range ) -> void
        {
            range_insert( std::ranges::begin( the_range ), static_cast<size_type>( std::ranges::distance( the_range ) ) );
        }

    private:
        auto remove_from_groups_with_erasures_list( group_pointer_type const group_to_remove ) noexcept -> void
        {
            if ( group_to_remove != erasure_groups_head )
            {
                group_to_remove->erasures_list_previous_group->erasures_list_next_group = group_to_remove->
                        erasures_list_next_group;

                if ( group_to_remove->erasures_list_next_group != nullptr )
                {
                    group_to_remove->erasures_list_next_group->erasures_list_previous_group = group_to_remove->
        ...
        auto reset_only_group_left( group_pointer_type const group_pointer ) noexcept -> void
        {
            erasure_groups_head = nullptr;
        ...
        auto add_group_to_unused_groups_list( group* const group_pointer ) noexcept -> void
        {
    ...
    public:
            ...
            // if uninitialized/invalid iterator supplied, function could generate an exception
        {
            ...
            if ( --( it.group_pointer->size ) != 0 ) // ie. non-empty group at this point in time, don't consolidate
            {
                ...
                char const prev_skipfield = *( it.skipfield_pointer - ( it.skipfield_pointer != it.group_pointer->skipfield ) ) !=
                                            0; // true if previous node is erased or this node is at beginning of skipfield
                char const after_skipfield = *( it.skipfield_pointer + 1 ) != 0;
                ...
                if ( !( prev_skipfield | after_skipfield ) ) // no consecutive erased elements
                {
                    *it.skipfield_pointer      = 1; // solo skipped node
                    skipfield_type const index = static_cast<skipfield_type>( it.element_pointer - to_aligned_pointer(
                    ...
                    {
                        edit_free_list_next(
                            to_aligned_pointer( it.group_pointer->elements ) + it.group_pointer->free_list_head,
                ...
                else if ( prev_skipfield & ( !after_skipfield ) ) // previous erased consecutive elements, none following
                {
                    *( it.skipfield_pointer - *( it.skipfield_pointer - 1 ) ) =
                            *it.skipfield_pointer = static_cast<skipfield_type>( *( it.skipfield_pointer - 1 ) + 1 );
                }
                else if ( ( !prev_skipfield ) & after_skipfield ) // following erased consecutive elements, none preceding
                {
                    skipfield_type const following_value = static_cast<skipfield_type>( *( it.skipfield_pointer + 1 ) + 1 );
                    *( it.skipfield_pointer + following_value - 1 ) = *( it.skipfield_pointer ) = following_value;

                    skipfield_type const following_previous = *( pointer_cast<skipfield_pointer_type>( it.element_pointer + 1 ) );
                    skipfield_type const following_next = *( pointer_cast<skipfield_pointer_type>( it.element_pointer + 1 ) + 1 );
                    ...
                    {
                        edit_free_list_next( to_aligned_pointer( it.group_pointer->elements ) + following_previous, index );
                    ...
                    {
                        edit_free_list_prev( to_aligned_pointer( it.group_pointer->elements ) + following_next, index );
                ...
                else // both preceding and following consecutive erased elements - erased element is between two skipblocks
                {
                    *( it.skipfield_pointer ) = 1;
                    // This line necessary in order for get_iterator() to work - ensures that erased element skipfield nodes are always non-zero
                    skipfield_type const preceding_value = *( it.skipfield_pointer - 1 );
                    skipfield_type const following_value = static_cast<skipfield_type>( *( it.skipfield_pointer + 1 ) + 1 );

                    // Join the skipblocks
                    *( it.skipfield_pointer - preceding_value )             =
                            *( it.skipfield_pointer + following_value - 1 ) = static_cast<skipfield_type>(
                    ...
                    {
                        edit_free_list_next(
                            to_aligned_pointer( it.group_pointer->elements ) + following_previous,
                ...
                iterator return_iterator(
                    it.group_pointer, it.element_pointer + update_value, it.skipfield_pointer + update_value );

                if ( return_iterator.element_pointer == pointer_cast<aligned_pointer_type>( it.group_pointer->skipfield ) && it.
                     group_pointer != end_iterator.group_pointer )
                {
                    return_iterator.group_pointer          = it.group_pointer->next_group;
                    aligned_pointer_type const elements    = to_aligned_pointer( return_iterator.group_pointer->elements );
                    skipfield_pointer_type const skipfield = return_iterator.group_pointer->skipfield;
                    return_iterator.element_pointer        = elements + *skipfield;
                    return_iterator.skipfield_pointer      = skipfield + *skipfield;
                }

                if ( it.element_pointer == begin_iterator.element_pointer ) begin_iterator = return_iterator;
            ...
            bool const in_back_block = ( it.group_pointer->next_group == nullptr ), in_front_block = (
            ...
            else if ( ( !in_back_block ) & in_front_block ) // ie. Remove first group, change first group to next group
            {
                ...
                begin_iterator.element_pointer = to_aligned_pointer( begin_iterator.group_pointer->elements ) + *( begin_iterator.
                                                     group_pointer->skipfield );
                // If the beginning index has been erased (ie. skipfield != 0), skip to next non-erased element
                begin_iterator.skipfield_pointer = begin_iterator.group_pointer->skipfield + *( begin_iterator.group_pointer->
                                                       skipfield );
            ...
            // this is a non-first group but not final group in chain: delete the group, then link previous group to the next group in the chain:
            {
                ...
                group_pointer_type const return_group                =
                        it.group_pointer->previous_group->next_group = it.group_pointer->next_group;
                ...
                return iterator(
                    return_group, to_aligned_pointer( return_group->elements ) + *( return_group->skipfield ),
            ...
            else // this is a non-first group and the final group in the chain
            {
                ...
                end_iterator.group_pointer                   = it.group_pointer->previous_group;
                // end iterator needs to be changed as element supplied was the back element of the hive
        ...
        auto erase(
            const_iterator const iterator1, const_iterator const iterator2 ) -> iterator
            // if uninitialized/invalid iterators supplied, function could generate an exception. If iterator1 > iterator2, behaviour is undefined.
        {
            ...
            {
                if ( current.element_pointer != to_aligned_pointer( current.group_pointer->elements ) + *( current.group_pointer->
                         skipfield ) ) // if iterator1 is not the first non-erased element in it's group - most common case
                {
                    size_type number_of_group_erasures = 0;

                    // Now update skipfield:
                    aligned_pointer_type const end = pointer_cast<aligned_pointer_type>( iterator1.group_pointer->skipfield );
                    ...
                    else
                    {
                        while ( current.element_pointer != end )
                        {
                            ...
                            else // remove skipblock from group:
                            {
                                skipfield_type const prev_free_list_index = *( pointer_cast<skipfield_pointer_type>(
                                    current.element_pointer ) );
                                skipfield_type const next_free_list_index = *( pointer_cast<skipfield_pointer_type>(
                                ...
                                if ( next_free_list_index == std::numeric_limits<skipfield_type>::max( ) && prev_free_list_index
                                     == std::numeric_limits<skipfield_type>::max( ) )
                                ...
                                // if this is the head of the free list
                                {
                                    ...
                                    edit_free_list_next(
                                        to_aligned_pointer( current.group_pointer->elements ) + prev_free_list_index,
                                        std::numeric_limits<skipfield_type>::max( ) );
                                }
                                else // either a tail or middle free list node
                                {
                                    edit_free_list_prev(
                                        to_aligned_pointer( current.group_pointer->elements ) + next_free_list_index,
                    ...
                    skipfield_type const previous_node_value = *( iterator1.skipfield_pointer - 1 );
                    // safe to do this here as we've already established that we're not at start of skipfield
                    skipfield_type const distance_to_end = static_cast<skipfield_type>( end - iterator1.element_pointer );
                    ...
                    if ( previous_node_value == 0 ) // no previous skipblock
                    {
                        ...
                        *( iterator1.skipfield_pointer + distance_to_end - 1 ) = distance_to_end; // set end node value
                        ...
                        skipfield_type const index = static_cast<skipfield_type>( iterator1.element_pointer - to_aligned_pointer(
                        ...
                        {
                            edit_free_list_next(
                                to_aligned_pointer( iterator1.group_pointer->elements ) + iterator1.group_pointer->free_list_head,
                                index ); // set prev free list head's 'next index' number to the index of the iterator1 element
                        }
                        else
                        {
                            iterator1.group_pointer->erasures_list_next_group = erasure_groups_head;
                            // add it to the groups-with-erasures free list
                    ...
                    else
                    {
                        ...
                        *( iterator1.skipfield_pointer - previous_node_value )         =
                                *( iterator1.skipfield_pointer + distance_to_end - 1 ) = static_cast<skipfield_type>(
                                    previous_node_value + distance_to_end );
                    }

                    if ( distance_to_end > 2 )
                    // if the skipblock is longer than 2 nodes, fill in the middle nodes with non-zero values so that get_iterator()
                    {
                        std::memset(
                            static_cast<void*>( std::to_address( iterator1.skipfield_pointer ) + 1 ), 1,
                ...
                // Intermediate groups:
                group_pointer_type const previous_group = current.group_pointer->previous_group;

                while ( current.group_pointer != iterator2.group_pointer )
                {
                    if constexpr ( !std::is_trivially_destructible<element_type>::value )
                    {
                        current.element_pointer = to_aligned_pointer( current.group_pointer->elements ) + *( current.group_pointer
                                                      ->skipfield );
                        current.skipfield_pointer      = current.group_pointer->skipfield + *( current.group_pointer->skipfield );
                        aligned_pointer_type const end = pointer_cast<aligned_pointer_type>( current.group_pointer->skipfield );
                    ...
                    group_pointer_type const current_group = current.group_pointer;
                    current.group_pointer                  = current.group_pointer->next_group;
            ...
            {
                if ( iterator2.element_pointer != end_iterator.element_pointer || current.element_pointer !=
                     to_aligned_pointer( current.group_pointer->elements ) + *( current.group_pointer->skipfield ) )
                // ie. not erasing entire group. Note: logistically the only way the entire group can be erased here is if iterator2 == end - otherwise would be caught by the if block above. Second condition in this if statement only possibly applies if iterator1.group_pointer == iterator2.group_pointer
                {
                    size_type number_of_group_erasures = 0;
                    ...
                    const_iterator const current_saved = current;
                    ...
                    skipfield_type const distance_to_iterator2 = static_cast<skipfield_type>(
                        iterator2.element_pointer - current_saved.element_pointer );
                    skipfield_type const index = static_cast<skipfield_type>( current_saved.element_pointer - to_aligned_pointer(
                    ...
                    // element is either at start of group or previous skipfield node is 0
                    {
                        ...
                        *( iterator2.skipfield_pointer - 1 ) = distance_to_iterator2;
                        ...
                        {
                            edit_free_list_next(
                                to_aligned_pointer( iterator2.group_pointer->elements ) + iterator2.group_pointer->free_list_head,
                                index );
                        }
                        else
                        {
                            iterator2.group_pointer->erasures_list_next_group = erasure_groups_head;
                            // add it to the groups-with-erasures free list
                    ...
                    // If iterator 1 & 2 are in same group, but iterator 1 was not at start of group, and previous skipfield node is an end node in a skipblock:
                    {
                        ...
                        skipfield_type const prev_node_value                   = *( current_saved.skipfield_pointer - 1 );
                        *( current_saved.skipfield_pointer - prev_node_value ) = static_cast<skipfield_type>(
                            prev_node_value + distance_to_iterator2 );
                        *( iterator2.skipfield_pointer - 1 ) = static_cast<skipfield_type>(
                            prev_node_value + distance_to_iterator2 );
                    }


                    if ( distance_to_iterator2 > 2 )
                    // if the skipblock is longer than 2 nodes, fill in the middle nodes with non-zero values so that get_iterator() will work
                    {
                        std::memset(
                            static_cast<void*>( std::to_address( current_saved.skipfield_pointer ) + 1 ), 1,
                    ...
                    if ( iterator1.element_pointer == begin_iterator.element_pointer )
                    {
                        begin_iterator = iterator(
                            iterator2.group_pointer, iterator2.element_pointer, iterator2.skipfield_pointer );
                    }
                ...
                else // ie. full group erasure
                {
                    ...
                    if ( ( total_size -= current.group_pointer->size ) != 0 ) // ie. previous_group != nullptr - hive is not empty
                    {
                        ...
                        end_iterator.element_pointer = pointer_cast<aligned_pointer_type>(
                            end_iterator.group_pointer->skipfield );
                        end_iterator.skipfield_pointer =
                                end_iterator.group_pointer->skipfield + end_iterator.group_pointer->capacity;
    ...
    private:
        auto prepare_groups_for_assign( size_type const size ) -> void
        {
            ...
            if ( size < total_capacity && ( total_capacity - size ) >= min_block_capacity )
            {
                ...
                do
                {
                    group_pointer_type const next_group = current_group->next_group;
    ...
    public:
        ...
        auto assign( size_type size, element_type const& element ) -> void
        {
            if ( size == 0 )
            {
                reset( );
            ...
            if constexpr ( ( std::is_trivially_destructible<element_type>::value && std::is_trivially_constructible<
                                 element_type>::value && std::is_trivially_copy_assignable<element_type>::value ) || !
                           std::is_copy_assignable<element_type>::value )
            // ie. If there is no benefit nor difference to assigning vs constructing, or if we can't assign, use faster method:
            {
                prepare_groups_for_assign( size );
                fill_unused_groups( size, element, 0, nullptr, begin_iterator.group_pointer );
            }
    ...
    private:
        auto destruct_remainder_range_assign( iterator it ) noexcept -> void
        {
            if constexpr ( !std::is_trivially_destructible<element_type>::value ) // avoid codegen if false
            {
        ...
        auto reset_group_range_assign( iterator& it ) noexcept -> void
        {
            std::memset(
                static_cast<void*>( it.group_pointer->skipfield ), 0, it.group_pointer->capacity * sizeof( skipfield_type ) );
            it.group_pointer->size = static_cast<skipfield_type>( it.element_pointer - to_aligned_pointer(
                                                                      it.group_pointer->elements ) );
        }


        auto make_back_group_range_assign( iterator& it ) noexcept -> void
        {
        ...
        auto finish_range_assign( iterator& it ) noexcept -> void
        {
        ...
        auto check_iterator_end_of_block( iterator& it ) noexcept -> void
        {
            if constexpr ( !std::is_trivially_destructible<element_type>::value )
            {
                if ( it.element_pointer == to_aligned_pointer( it.group_pointer->skipfield ) )
                {
                    ...
                    skipfield_type const skip = *( it.group_pointer->skipfield );
        ...
        template <class iterator_type>
        auto range_assign( iterator_type it, size_type size ) -> void
        {
            if ( size == 0 )
            {
                reset( );
            ...
            else
            {
                ...
                for ( iterator current( begin_iterator ); current.group_pointer != nullptr; )
                {
                    ...
                    for ( aligned_pointer_type const end = ( current.group_pointer == end_iterator.group_pointer )
                                                               ? end_iterator.element_pointer
                                                               : to_aligned_pointer(
                                                                   current.group_pointer->skipfield );
                          current.element_pointer != end; )
                    {
                        if ( *( current.skipfield_pointer ) != 0 )
                        {
                            skipfield_type skipblock_length = *( current.skipfield_pointer );
                            iterator next_element(
                                current.group_pointer, current.element_pointer + skipblock_length,
                                current.skipfield_pointer + skipblock_length );
                            skipblock_length = ( skipblock_length > size )
                                                   ? static_cast<skipfield_type>( size )
                                                   : skipblock_length;

                            for ( aligned_pointer_type const fill_end = current.element_pointer + skipblock_length;
                                  current.element_pointer != fill_end; ++current.element_pointer, ++current.skipfield_pointer )
                            {
#ifdef PLF_EXCEPTIONS_SUPPORT
                                if constexpr ( !std::is_nothrow_copy_constructible<element_type>::value )
                                {
                                    ...
                                    catch ( ... )
                                    {
                                        if constexpr ( !std::is_trivially_destructible<element_type>::value )
                                        {
                                            ...
                                            destruct_remainder_range_assign( next_element );
                                        }
                        ...
                        else
                        {
                            ...
                            if ( --size == 0 )
                            {
                                if constexpr ( !std::is_trivially_destructible<element_type>::value )
                                {
                                    destruct_remainder_range_assign( ++iterator( current ) );
                                }
                                // To potentially allow for skipping over a skipblock
                ...
                for ( aligned_pointer_type const end = to_aligned_pointer( end_iterator.group_pointer->skipfield );
                      end_iterator.element_pointer != end; )
                {
                    construct_element( end_iterator.element_pointer, *it++ );
                    ++total_size;
                    ++end_iterator.group_pointer->size;
    ...
    public:
        ...
        template <class iterator_type>
        auto assign( std::move_iterator<iterator_type> const first, std::move_iterator<iterator_type> const last ) -> void
        {
        ...
        auto assign( std::initializer_list<element_type> const& element_list ) -> void
        {
        ...
        template <class range_type>
            requires std::ranges::range<range_type>
        auto assign_range( range_type&& the_range ) -> void
        {
    ...
    private:
        ...
        auto consolidate( skipfield_type const new_min, skipfield_type const new_max ) -> void
        {
            ...
            if constexpr ( !std::is_trivially_copyable<element_type>::value && std::is_nothrow_move_constructible<
                               element_type>::value )
            {
                temp.range_fill_unused_groups(
                    total_size, std::make_move_iterator( begin_iterator ), 0, nullptr, temp.begin_iterator.group_pointer );
            }
            else
            {
    ...
    public:
        auto reshape( plf::hive_limits const block_limits ) -> void
        {
            check_capacities_conformance( block_limits );
            skipfield_type const new_min = static_cast<skipfield_type>( block_limits.min ), new_max = static_cast<skipfield_type>(
                                     block_limits.max );
            ...
            if ( total_capacity != 0 )
            {
                if ( total_size != 0 )
                {
                    if ( min_block_capacity > new_max || max_block_capacity < new_min )
                    // If none of the original blocks could potentially fit within the new limits, skip checking of blocks and just consolidate:
                    {
                        consolidate( new_min, new_max );
                        return;
                    }

                    if ( min_block_capacity < new_min || max_block_capacity > new_max )
                    // ie. If existing blocks could be outside of the new limits
                    {
                        ...
                        for ( group_pointer_type current_group = begin_iterator.group_pointer; current_group != nullptr;
                              current_group                    = current_group->next_group )
                        {
        ...
        constexpr auto block_capacity_limits( ) const noexcept -> hive_limits
        {
            return hive_limits( static_cast<size_t>( min_block_capacity ), static_cast<size_t>( max_block_capacity ) );
        }


        static constexpr auto block_capacity_hard_limits( ) noexcept -> hive_limits
        {
            return hive_limits(
                3, std::min( static_cast<size_t>( std::numeric_limits<skipfield_type>::max( ) ), max_size_static( ) ) );
        }
        ...
        auto operator =( hive const& source ) -> hive&
        {
            assert( &source != this );
            ...
            if constexpr ( std::allocator_traits<allocator_type>::propagate_on_container_copy_assignment::value )
            {
                if constexpr ( !std::allocator_traits<allocator_type>::is_always_equal::value )
                {
                    ...
                    if ( static_cast<allocator_type&>( *this ) != static_cast<allocator_type const&>( source ) )
                    {
                        reset( );
                    }
                }

                static_cast<allocator_type&>( *this ) = static_cast<allocator_type const&>( source );
                // Reconstruct rebinds:
    ...
    private:
        auto move_assign(
            hive&& source ) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value ||
                                     std::allocator_traits<allocator_type>::is_always_equal::value) -> void
        {
            destroy_all_data( );

            if constexpr ( ( std::is_trivially_copyable<allocator_type>::value || std::allocator_traits<
                                 allocator_type>::is_always_equal::value ) && std::is_trivially_copyable<
                               group_pointer_type>::value )
            {
    ...
    public:
        ...
        auto operator =(
            hive&& source ) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value ||
                                     std::allocator_traits<allocator_type>::is_always_equal::value) -> hive&
        {
            assert( &source != this );
            ...
            {
                if constexpr ( !( std::is_move_constructible<element_type>::value && std::is_move_assignable<
                                      element_type>::value ) )
                {
                    range_assign( source.begin_iterator, source.total_size );
                }
        ...
        auto operator =( std::initializer_list<element_type> const& element_list ) -> hive&
        {
            range_assign( element_list.begin( ), static_cast<size_type>( element_list.size( ) ) );
            return *this;
        }
        ...
        auto trim_capacity( ) noexcept -> void
        {
            if ( end_iterator.element_pointer == nullptr ) return; // empty hive

            while ( unused_groups_head != nullptr )
            {
                group_pointer_type const next_group = unused_groups_head->next_group;
        ...
        auto trim_capacity( size_type const capacity_retain ) noexcept -> void
        {
            size_type const capacity_difference = total_capacity - capacity_retain;

            if ( end_iterator.element_pointer == nullptr || total_capacity <= capacity_retain || total_size >= capacity_retain ||
            ...
            if ( begin_iterator.element_pointer == end_iterator.element_pointer ) // ie. clear() has been called prior
            {
                if ( number_of_elements_to_remove >= begin_iterator.group_pointer->capacity )
                {
                    ...
                    if ( unused_groups_head != nullptr )
                    // some of the reserved blocks were not removed as they were too large, so use one of these to make the new begin group
                    {
                        begin_iterator.group_pointer     = unused_groups_head;
        ...
        auto reserve( size_type new_capacity ) -> void
        {
            ...
            else
            {
                ...
                if ( unused_groups_head != nullptr && max_block_capacity - remainder >= min_block_capacity )
                {
                    ...
                    do
                    {
                        skipfield_type const current_capacity = deallocatable_group->capacity;
                        ...
                        if ( std::numeric_limits<skipfield_type>::max( ) - current_capacity > remainder &&
                             /* <- to make sure we don't overflow in next line */
                             max_block_capacity >= current_capacity + remainder )
                        {
                            remainder += current_capacity;
                            group_pointer_type const next_group = deallocatable_group->next_group;
                ...
                // Implies we were unable to consolidate remainder with an existing unused group, in the if-block above
                {
                    ...
                    if ( max_size( ) - total_capacity < ( ( number_of_max_groups + 1 ) * min_block_capacity ) )
                    {
                        ...
                        throw std::length_error(
                            "Reserve cannot increase capacity to >= n without being > max_size() due to current capacity() and block capacity limits" );
#else
                        std::terminate( );
#endif
                    }
            ...
            if ( begin_iterator.group_pointer == nullptr ) // Most common scenario - uninitialized container
            {
                ...
                begin_iterator.group_pointer->size = 0;
                // Note: this is set to 1 by default in the initialize function (which is optimised for insert())

                if ( number_of_max_groups == 0 ) return;

                // Make the first allocated unused group:
                skipfield_type const new_block_capacity = ( max_block_capacity - negative_remainder < min_block_capacity )
                                                              ? min_block_capacity
    ...
    private:
        ...
        auto get_it( pointer const element_pointer ) const noexcept -> hive_iterator<is_const>
        {
            if ( end_iterator.group_pointer != nullptr )
            {
                aligned_pointer_type const aligned_element_pointer = pointer_cast<aligned_pointer_type>( element_pointer );
                ...
                if ( std::greater_equal( )( aligned_element_pointer, to_aligned_pointer( end_iterator.group_pointer->elements ) )
                     && std::less( )( aligned_element_pointer, end_iterator.element_pointer ) )
                {
                    skipfield_pointer_type const skipfield_pointer =
                            end_iterator.group_pointer->skipfield + ( aligned_element_pointer - to_aligned_pointer(
                                                                          end_iterator.group_pointer->elements ) );
                    return ( *skipfield_pointer == 0 )
                               ? hive_iterator<is_const>( end_iterator.group_pointer, aligned_element_pointer, skipfield_pointer )
                               : end_iterator;
                }

                // All other groups, if any exist:
                for ( group_pointer_type current_group = end_iterator.group_pointer->previous_group; current_group != nullptr;
                      current_group                    = current_group->previous_group )
                {
                    if ( std::greater_equal( )( aligned_element_pointer, to_aligned_pointer( current_group->elements ) ) &&
                         std::less( )( aligned_element_pointer, pointer_cast<aligned_pointer_type>( current_group->skipfield ) ) )
                    {
                        skipfield_pointer_type const skipfield_pointer =
                                current_group->skipfield + ( aligned_element_pointer - to_aligned_pointer(
                                                                 current_group->elements ) );
                        return ( *skipfield_pointer == 0 )
                                   ? hive_iterator<is_const>( current_group, aligned_element_pointer, skipfield_pointer )
                                   : end_iterator;
                    }
                }
            }
    ...
    public:
        auto get_iterator( pointer const element_pointer ) noexcept -> iterator
        {
            return get_it<false>( element_pointer );
        }


        auto get_iterator( const_pointer const element_pointer ) const noexcept -> const_iterator
        {
    ...
    private:
        auto source_blocks_incompatible( ) -> void
        {
#ifdef PLF_EXCEPTIONS_SUPPORT
            throw std::length_error(
    ...
    public:
        auto splice( hive& source ) -> void
        {
            ...
            if ( source.min_block_capacity > max_block_capacity || source.max_block_capacity < min_block_capacity )
            // ie. source blocks cannot possibly fit within *this's block capacity limits
            {
                source_blocks_incompatible( );
            }
            else if ( source.min_block_capacity < min_block_capacity || source.max_block_capacity > max_block_capacity )
            // ie. source blocks may or may not fit
            {
                for ( group_pointer_type current_group = source.begin_iterator.group_pointer; current_group != nullptr;
                      current_group                    = current_group->next_group )
                {
            ...
            if ( total_size != 0 )
            {
                ...
                if ( ( pointer_cast<aligned_pointer_type>( end_iterator.group_pointer->skipfield ) - end_iterator.
                       element_pointer ) > ( pointer_cast<aligned_pointer_type>( source.end_iterator.group_pointer->skipfield ) -
                ...
                skipfield_type const distance_to_end = static_cast<skipfield_type>( pointer_cast<aligned_pointer_type>(
                                                                                        end_iterator.group_pointer->skipfield ) -
                                                                                    end_iterator.element_pointer );
                ...
                if ( distance_to_end != 0 ) // 0 == edge case
                {
                    ...
                    // Update skipfield:
                    skipfield_type const previous_node_value = *( end_iterator.skipfield_pointer - 1 );
                    ...
                    if ( previous_node_value == 0 ) // no previous skipblock
                    {
                        ...
                        *( end_iterator.skipfield_pointer + distance_to_end - 1 ) = distance_to_end;
                        ...
                        {
                            std::memset(
                                static_cast<void*>( end_iterator.skipfield_pointer + 1 ), 1,
                                sizeof( skipfield_type ) * ( distance_to_end - 2 ) );
                        }

                        skipfield_type const index = static_cast<skipfield_type>(
                            end_iterator.element_pointer - to_aligned_pointer( end_iterator.group_pointer->elements ) );
                        ...
                        {
                            edit_free_list_next(
                                to_aligned_pointer( end_iterator.group_pointer->elements ) + end_iterator.group_pointer->
                                free_list_head,
                                index ); // set prev free list head's 'next index' number to the index of the current element
                        }
                        else
                        {
                            end_iterator.group_pointer->erasures_list_next_group = erasure_groups_head;
                            // add it to the groups-with-erasures free list
                    ...
                    else
                    {
                        ...
                        *( end_iterator.skipfield_pointer - previous_node_value )         =
                                *( end_iterator.skipfield_pointer + distance_to_end - 1 ) = static_cast<skipfield_type>(
                                    previous_node_value + distance_to_end );

                        if ( distance_to_end > 1 ) // make erased middle nodes non-zero for get_iterator
                        {
                            std::memset(
                                static_cast<void*>( end_iterator.skipfield_pointer ), 1,
                ...
                if ( source.begin_iterator.group_pointer->group_number <= end_iterator.group_pointer->group_number )
                {
                    ...
                    for ( group_pointer_type current_group = source.begin_iterator.group_pointer; current_group != nullptr;
                          current_group                    = current_group->next_group, ++source_group_count ) { }

                    if ( ( std::numeric_limits<size_type>::max( ) - end_iterator.group_pointer->group_number ) >=
                         source_group_count )
                    {
                        update_subsequent_group_numbers(
                            end_iterator.group_pointer->group_number + 1u, source.begin_iterator.group_pointer );
                    }
                    else [[unlikely]]
                    {
                        reset_group_numbers( );
            ...
            else // If *this is empty():
            {
                ...
                group_pointer_type const original_unused_groups = unused_groups_head;
                unused_groups_head                              = nullptr;
                destroy_all_data( );
            ...
            if ( original_unused_groups_head != nullptr )
            // If there were unused groups in source, re-link them and remove their capacity count from *this while adding it to source:
            {
                size_type source_unused_groups_capacity = 0;

                // Count capacity in source unused_groups:
                for ( group_pointer_type current = original_unused_groups_head; current != nullptr;
    ...
    private:
        struct item_index_tuple
        {
            ...
            item_index_tuple( pointer const _item, size_type const _index ) noexcept
        ...
        struct sort_dereferencer
        {
            ...
            explicit sort_dereferencer( comparison_function const& function_instance ) : stored_instance( function_instance ) { }


            auto operator()( item_index_tuple const first, item_index_tuple const second ) -> bool
            {
        ...
        auto get_free_space( ) const noexcept -> aligned_pointer_type
        {
            size_type const number_of_elements_needed =
                    ( ( total_size * sizeof( the_type ) ) + sizeof( aligned_element_struct ) - 1 ) / sizeof(
                        aligned_element_struct ); // rounding up

            if ( number_of_elements_needed < max_block_capacity )
            {
                if ( static_cast<size_type>( pointer_cast<aligned_pointer_type>( end_iterator.group_pointer->skipfield ) -
                                             end_iterator.element_pointer ) >= number_of_elements_needed )
                {
    ...
    public:
        ...
        auto sort( comparison_function compare = comparison_function( ) ) -> void
        {
            ...
            if constexpr ( ( std::is_trivially_copyable<element_type>::value || std::is_move_assignable<element_type>::value ) &&
                           sizeof( element_type ) <= sizeof( pointer ) * 2 )
            // If element is <= 2 pointers, just copy to an array and sort that then copy back - consumes less memory and may be faster
            {
                pointer sort_array          = pointer_cast<pointer>( get_free_space<element_type>( ) );
                bool const need_to_allocate = ( sort_array == nullptr );
                ...
                pointer const end = sort_array + total_size;

                if constexpr ( !std::is_trivially_copyable<element_type>::value && std::is_move_assignable<element_type>::value )
                {
                    std::uninitialized_copy(
                        std::make_move_iterator( begin_iterator ), std::make_move_iterator( end_iterator ), sort_array );
                }
                else
                {
            ...
            else
            {
                ...
                if ( need_to_allocate )
                {
                    sort_array = std::allocator_traits<tuple_allocator_type>::allocate(
                        tuple_allocator, total_size, end_iterator.skipfield_pointer );
                }
                ...
                for ( tuple_pointer_type current_tuple = sort_array; current_tuple != tuple_pointer; ++current_tuple, ++index )
                {
                    if ( current_tuple->original_index != index )
                    {
                        ...
                        do
                        {
                            *( sort_array[destination_index].original_location ) = std::move(
        ...
        auto unique( comparison_function compare = comparison_function( ) ) -> size_type
        {
            if ( total_size < 2 ) return 0;

            size_type count          = 0;
            const_iterator const end = end_iterator;
            ...
            for ( const_iterator current = begin_iterator, previous = begin_iterator; ++current != end; previous = current )
            {
                ...
                {
                    size_type const original_count = ++count;
        ...
        auto swap(
            hive& source ) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_swap::value ||
                                    std::allocator_traits<allocator_type>::is_always_equal::value) -> void
        {
            assert( &source != this );

            if constexpr ( std::allocator_traits<allocator_type>::is_always_equal::value && std::is_trivially_copyable<
                               group_pointer_type>::value )
            ...
            else if constexpr ( std::is_move_assignable<group_pointer_type>::value && std::is_move_constructible<
                                    group_pointer_type>::value )
            {
            ...
            else
            {
                ...
                iterator const swap_end_iterator                  = end_iterator, swap_begin_iterator            = begin_iterator;
                group_pointer_type const swap_erasure_groups_head = erasure_groups_head, swap_unused_groups_head =
                                                 unused_groups_head;
                size_type const swap_total_size              = total_size, swap_total_capacity             = total_capacity;
                skipfield_type const swap_min_block_capacity = min_block_capacity, swap_max_block_capacity = max_block_capacity;
                ...
                if constexpr ( std::allocator_traits<allocator_type>::propagate_on_container_swap::value && !std::allocator_traits
        ...
        {
        private:
            using group_pointer_type     = typename hive::group_pointer_type;
            using aligned_pointer_type   = typename hive::aligned_pointer_type;
            using skipfield_pointer_type = typename hive::skipfield_pointer_type;
        ...
        public:
            ...
            using reverse_type = hive_reverse_iterator<is_const>;
            using pointer = typename std::conditional_t<is_const, typename hive::const_pointer, typename hive::pointer>;
            using reference = typename std::conditional_t<is_const, typename hive::const_reference, typename hive::reference>;
            ...
            hive_iterator( hive_iterator const& source ) noexcept
                : // Note: Surprisingly, use of = default here and in other simple constructors results in slowdowns of up to 10% in many benchmarks under GCC
            ...
            hive_iterator( hive_iterator<false> const& source ) noexcept
                : group_pointer( source.group_pointer )
                , element_pointer( source.element_pointer )
                , skipfield_pointer( source.skipfield_pointer ) { }
            ...
            auto operator =( hive_iterator const& source ) noexcept -> hive_iterator&
            {
                group_pointer     = source.group_pointer;
                element_pointer   = source.element_pointer;
                skipfield_pointer = source.skipfield_pointer;
            ...
            template <bool is_const_it = is_const, class = std::enable_if_t<is_const_it>>
            auto operator =( hive_iterator<false> const& source ) noexcept -> hive_iterator&
            {
                group_pointer     = source.group_pointer;
                element_pointer   = source.element_pointer;
                skipfield_pointer = source.skipfield_pointer;
            ...
            auto operator =( hive_iterator&& source ) noexcept -> hive_iterator&
            {
                assert( &source != this );
                group_pointer     = std::move( source.group_pointer );
                element_pointer   = std::move( source.element_pointer );
            ...
            template <bool is_const_it = is_const, class = std::enable_if_t<is_const_it>>
            auto operator =( hive_iterator<false>&& source ) noexcept -> hive_iterator&
            {
                group_pointer     = std::move( source.group_pointer );
                element_pointer   = std::move( source.element_pointer );
                skipfield_pointer = std::move( source.skipfield_pointer );
            ...
            auto operator ==( hive_iterator const& rh ) const noexcept -> bool
            {
                return ( element_pointer == rh.element_pointer );
            }


            auto operator ==( hive_iterator<!is_const> const& rh ) const noexcept -> bool
            {
                return ( element_pointer == rh.element_pointer );
            }
            ...
            auto operator *( ) const -> reference // may cause exception with uninitialized iterator
            {
            ...
            auto operator ++( ) -> hive_iterator&
            {
                assert( group_pointer != nullptr ); // covers uninitialised hive_iterator
                skipfield_type skip = *( ++skipfield_pointer );

                if ( ( element_pointer += static_cast<size_type>( skip ) + 1u ) == pointer_cast<
                         aligned_pointer_type>( group_pointer->skipfield ) && group_pointer->next_group != nullptr )
                // ie. beyond end of current memory block. Second condition allows iterator to reach end(), which may be 1 past end of block, if block has been fully used and another block is not allocated
                {
                    ...
                    aligned_pointer_type const elements    = to_aligned_pointer( group_pointer->elements );
                    skipfield_pointer_type const skipfield = group_pointer->skipfield;
                    skip                                   = *skipfield;
            ...
            auto operator ++( int ) -> hive_iterator
            {
                hive_iterator const copy( *this );
                ++*this;
                return copy;
            }
            ...
            auto operator --( ) -> hive_iterator&
            {
                ...
                // ie. not already at beginning of group prior to decrementation
                {
                    ...
                    // ie. skipfield jump value does not takes us beyond beginning of group
                }

                group_pointer = group_pointer->previous_group;
                skipfield_pointer_type const skipfield = group_pointer->skipfield + group_pointer->capacity - 1;
                skipfield_type const skip = *skipfield;
            ...
            auto operator >( hive_iterator<is_const_it> const& rh ) const noexcept -> bool
            {
            ...
            template <bool is_const_it>
            auto operator <=>( hive_iterator<is_const_it> const& rh ) const noexcept -> std::strong_ordering
            {
                return ( element_pointer == rh.element_pointer )
        ...
        private:
            ...
            hive_iterator(
                group_pointer_type const group_p, aligned_pointer_type const element_p,
                skipfield_pointer_type const skipfield_p ) noexcept
                ...
                // Cannot be noexcept due to the possibility of an uninitialized iterator
            {
                ...
                if ( distance > 0 ) // ie. +=
                {
                    ...
                    if ( group_pointer->next_group == nullptr && element_pointer == pointer_cast<aligned_pointer_type>(
                             group_pointer->skipfield ) )
                    ...
                    if ( element_pointer != to_aligned_pointer( group_pointer->elements ) + *( group_pointer->skipfield ) )
                    // ie. != first non-erased element in group - otherwise we skip this section and just treat the first block as we would an intermediary block
                    {
                        difference_type const distance_from_end =
                                pointer_cast<aligned_pointer_type>( group_pointer->skipfield ) - element_pointer;
                        ...
                        // ie. if there are no erasures in the group
                        {
                            ...
                            // either we've reached end() or gone beyond it, so bound to back of block
                            {
                        ...
                        {
                            skipfield_pointer_type const endpoint = skipfield_pointer + distance_from_end;
                            ...
                            // either we've reached end() or gone beyond it, so bound to end of block
                            {
                    ...
                    // We already know size > distance due to the intermediary group checks above - safe to ignore endpoint check condition while incrementing here:
                    {
                        skipfield_pointer = group_pointer->skipfield + *( group_pointer->skipfield );
                ...
                else if ( distance < 0 )
                {
                    ...
                    if ( group_pointer->previous_group == nullptr && element_pointer == to_aligned_pointer(
                             group_pointer->elements ) + *( group_pointer->skipfield ) )
                        return;
                    // check if we're already at begin()
                    distance = -distance;

                    // Special case for initial element pointer and initial group (we don't know how far into the group the element pointer is)
                    if ( element_pointer != pointer_cast<aligned_pointer_type>( group_pointer->skipfield ) )
                    // not currently at the back of a block
                    {
                        if ( group_pointer->free_list_head == std::numeric_limits<skipfield_type>::max( ) )
                        // ie. no prior erasures have occurred in this group
                        {
                            difference_type const distance_from_beginning = static_cast<difference_type>(
                                element_pointer - to_aligned_pointer( group_pointer->elements ) );
                        ...
                        {
                            skipfield_pointer_type const beginning_point =
                    ...
                    {
                        skipfield_pointer = group_pointer->skipfield + ( pointer_cast<aligned_pointer_type>(
                                                                             group_pointer->skipfield ) - to_aligned_pointer(
            ...
            auto distance( hive_iterator const& last ) const -> difference_type
            {
                // Code logic:
                ...
                if ( iterator1.group_pointer != iterator2.group_pointer ) // if not in same group, process intermediate groups
                {
                    ...
                    {
                        distance += static_cast<difference_type>( pointer_cast<aligned_pointer_type>(
                                                                      iterator1.group_pointer->skipfield ) - iterator1.
                                                                  element_pointer );
                    }
                    else if ( iterator1.element_pointer == to_aligned_pointer( iterator1.group_pointer->elements ) + *( iterator1.
                                  group_pointer->skipfield ) ) // ie. element is at start of group - rare case
                    {
                        distance += static_cast<difference_type>( iterator1.group_pointer->size );
                    }
                    else // Manually iterate to find distance to end of group:
                    {
                        skipfield_pointer_type const endpoint =
                                iterator1.skipfield_pointer + ( pointer_cast<aligned_pointer_type>(
                                                                    iterator1.group_pointer->skipfield ) - iterator1.
                                                                element_pointer );
                ...
                else if ( iterator1.element_pointer == to_aligned_pointer( iterator2.group_pointer->elements ) + *( iterator2.
                              group_pointer->skipfield ) && iterator2.element_pointer + 1 + *( iterator2.skipfield_pointer + 1 )
                          == pointer_cast<aligned_pointer_type>( iterator2.group_pointer->skipfield ) )
                // ie. if iterator1 is at beginning of block (have to check this in case first and last are in the same block to begin with) and iterator2 is last element in the block
                {
        ...
        public:
            ...
            using value_type = typename hive::value_type;
            using difference_type = typename hive::difference_type;
            using pointer = typename std::conditional_t<is_const_r, typename hive::const_pointer, typename hive::pointer>;
            using reference = typename std::conditional_t<is_const_r, typename hive::const_reference, typename hive::reference>;
            ...
            hive_reverse_iterator( hive_reverse_iterator const& source ) noexcept : current( source.current ) { }


            template <bool is_const_rit = is_const_r, class = std::enable_if_t<is_const_rit>>
            hive_reverse_iterator( hive_reverse_iterator<false> const& source ) noexcept : current( source.current ) { }


            hive_reverse_iterator( hive_iterator<is_const_r> const& source ) noexcept
                : current( source )
            {
                ++( *this );
            }
            ...
            template <bool is_const_rit = is_const_r, class = std::enable_if_t<is_const_rit>>
            hive_reverse_iterator( hive_iterator<false> const& source ) noexcept
                : current( source )
            {
                ++( *this );
            }
            ...
            hive_reverse_iterator( hive_reverse_iterator&& source ) noexcept : current( std::move( source.current ) ) { }


            template <bool is_const_rit = is_const_r, class = std::enable_if_t<is_const_rit>>
            hive_reverse_iterator( hive_reverse_iterator<false>&& source ) noexcept : current( std::move( source.current ) ) { }


            auto operator =( hive_iterator<is_const_r> const& source ) noexcept -> hive_reverse_iterator&
            {
                current = source;
                ++( *this );
                return *this;
            ...
            template <bool is_const_rit = is_const_r, class = std::enable_if_t<is_const_rit>>
            auto operator =( hive_iterator<false> const& source ) noexcept -> hive_reverse_iterator&
            {
                current = source;
                ++( *this );
                return *this;
            ...
            auto operator =( hive_reverse_iterator const& source ) noexcept -> hive_reverse_iterator&
            {
            ...
            template <bool is_const_rit = is_const_r, class = std::enable_if_t<is_const_rit>>
            auto operator =( hive_reverse_iterator<false> const& source ) noexcept -> hive_reverse_iterator&
            {
                current = source.current;
                return *this;
            }
            ...
            auto operator =( hive_reverse_iterator&& source ) noexcept -> hive_reverse_iterator&
            {
                assert( &source != this );
            ...
            template <bool is_const_rit = is_const_r, class = std::enable_if_t<is_const_rit>>
            auto operator =( hive_reverse_iterator<false>&& source ) noexcept -> hive_reverse_iterator&
            {
                assert( &source != this );
                current = std::move( source.current );
                return *this;
            ...
            auto operator ==( hive_reverse_iterator const& rh ) const noexcept -> bool
            {
                return ( current == rh.current );
            }


            auto operator ==( hive_reverse_iterator<!is_const_r> const& rh ) const noexcept -> bool
            {
                return ( current == rh.current );
            }
            ...
            auto operator ++( ) -> hive_reverse_iterator&
            {
            ...
            auto operator ++( int ) -> hive_reverse_iterator
            {
                hive_reverse_iterator const copy( *this );
                ++*this;
                return copy;
            }
            ...
            auto base( ) const noexcept -> hive_iterator<is_const_r>
            {
            ...
            template <bool is_const_it>
            auto operator >( hive_reverse_iterator<is_const_it> const& rh ) const noexcept -> bool
            {
            ...
            template <bool is_const_it>
            auto operator <=>( hive_reverse_iterator<is_const_it> const& rh ) const noexcept -> std::strong_ordering
            {
                return ( rh.current <=> current );
            }
        ...
        private:
            ...
            hive_reverse_iterator(
                group_pointer_type const group_p, aligned_pointer_type const element_p,
                skipfield_pointer_type const skipfield_p ) noexcept : current( group_p, element_p, skipfield_p ) { }


            // distance implementation:

            auto distance( hive_reverse_iterator const& last ) const -> difference_type
            {
            ...
            auto advance( difference_type distance ) -> void
            {
                group_pointer_type& group_pointer         = current.group_pointer;
                aligned_pointer_type& element_pointer     = current.element_pointer;
                skipfield_pointer_type& skipfield_pointer = current.skipfield_pointer;
                ...
                if ( distance > 0 )
                {
                    ...
                    if ( group_pointer->free_list_head == std::numeric_limits<skipfield_type>::max( ) )
                    {
                        difference_type const distance_from_beginning = element_pointer - to_aligned_pointer(
                                                                            group_pointer->elements );
                    ...
                    {
                        skipfield_pointer_type const beginning_point = group_pointer->skipfield + *( group_pointer->skipfield );
                ...
                {
                    if ( group_pointer->next_group == nullptr && (
                             element_pointer == ( pointer_cast<aligned_pointer_type>( group_pointer->skipfield ) - 1 ) - *(
                                 group_pointer->skipfield + (
                                     pointer_cast<aligned_pointer_type>( group_pointer->skipfield ) - to_aligned_pointer(
                                         group_pointer->elements ) ) - 1 ) ) ) // Check if we're already at rbegin()
                    {
                        return;
                    }
                    ...
                    // ie. != first non-erased element in group
                    {
                        ...
                        // ie. if there are no erasures in the group
                        {
                            difference_type const distance_from_end = pointer_cast<aligned_pointer_type>(
                                                                          group_pointer->skipfield ) - element_pointer;
                        ...
                        {
                            skipfield_pointer_type const endpoint = skipfield_pointer + ( pointer_cast<aligned_pointer_type>(
                                                                        group_pointer->skipfield ) - element_pointer );
                    ...
                    // we already know size > distance from previous loop - so it's safe to ignore endpoint check condition while incrementing:
                    {
                        skipfield_pointer = group_pointer->skipfield + *( group_pointer->skipfield );
    ...
    struct hive_eq_to
    {
        ...
        explicit hive_eq_to( element_type const& store_value ) noexcept : value( store_value ) { }


        auto operator()( element_type const& compare_value ) const noexcept -> bool
        {
...
namespace std
{
    ...
    auto swap(
        plf::hive<element_type, allocator_type>& a,
        plf::hive<element_type, allocator_type>& b ) noexcept(
        std::allocator_traits<allocator_type>::propagate_on_container_swap::value || std::allocator_traits<
            allocator_type>::is_always_equal::value) -> void
    {
        a.swap( b );
    }
    ...
    auto erase_if(
        ...
        predicate_function predicate ) -> typename plf::hive<element_type, allocator_type>::size_type
    {
        using hive               = typename plf::hive<element_type, allocator_type>;
        using const_iterator     = typename hive::const_iterator;
        using size_type          = typename hive::size_type;
        size_type count          = 0;
        const_iterator const end = container.cend( );
    ...
    auto erase(
        plf::hive<element_type, allocator_type>& container,
        element_type const& value ) -> typename plf::hive<element_type, allocator_type>::size_type
    {
    ...
    {
    public:
        using rit = typename it_type::reverse_type;
        using rit::rit;
    };
} // namespace std
