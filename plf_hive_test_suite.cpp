...
#ifndef PLF_HIVE_TEST_SUITE_H
#define PLF_HIVE_TEST_SUITE_H
...
inline auto message( char const* message_text ) -> void
{
    printf( "%s\n", message_text );
}


inline auto title1( char const* title_text ) -> void
{
    printf( "\n\n\n*** %s ***\n", title_text );
    printf( "===========================================\n\n\n" );
}
...
inline auto title2( char const* title_text ) -> void
{
    printf( "\n\n--- %s ---\n\n", title_text );
}


inline auto failpass( char const* test_type, bool const condition ) -> void
{
    printf( "%s: ", test_type );
...
struct small_struct
{
    double* empty_field_1{ nullptr };
    double unused_number{ 0.f };
    unsigned int empty_field2{ 0U };
    double* empty_field_3{ nullptr };
    int number{ 0 };
    unsigned int empty_field4{ 0U };

    auto operator *( ) const -> int { return number; };
    auto operator ==( small_struct const& source ) const noexcept -> bool { return source.number == number; };
    auto operator !=( small_struct const& source ) const noexcept -> bool { return source.number != number; };
    auto operator >( small_struct const& source ) const noexcept -> bool { return number > source.number; };
    auto operator <( small_struct const& source ) const noexcept -> bool { return number < source.number; };
    auto operator >=( small_struct const& source ) const noexcept -> bool { return number >= source.number; };
    auto operator <=( small_struct const& source ) const noexcept -> bool { return number <= source.number; };

    small_struct( unsigned int const num ) noexcept : number( num ) { };
};
...
{
public:
    non_copyable_type( int const a ) : i_( a ) { }

private:
    int i_;
    non_copyable_type( non_copyable_type const& );                    // non construction-copyable
    auto operator=( non_copyable_type const& ) -> non_copyable_type&; // non copyable
};
...
struct small_struct_non_trivial
{
    ...
    small_struct_non_trivial( int const num ) : number( num ) { }
    ...
    {
        {
            ...
            for ( auto the_iterator = p_hive.begin( ); the_iterator != p_hive.end( ); ++the_iterator )
            {
                ++total;
            ...
            auto plus_twenty = p_hive.begin( );
            plf::advance( plus_twenty, 20 );
            auto plus_two_hundred = p_hive.begin( );
            plf::advance( plus_two_hundred, 200 );

            failpass( "Iterator + distance test", plf::distance( p_hive.begin( ), plus_twenty ) == 20 );
            failpass( "Iterator + distance test 2", plf::distance( p_hive.begin( ), plus_two_hundred ) == 200 ); {
                hive<int> d_hive( 1000, 1, { 20, 20 } );

                for ( auto current = d_hive.begin( ), end = d_hive.end( ); current != end; )
                {
                    if ( ( rand( ) & 7 ) == 0 )
                    {
                ...
                {
                    int const dist1           = rand( ) % ( d_size - 2 ), dist2 = rand( ) % ( ( d_size - 2 ) - dist1 );
                    hive<int>::iterator first = d_hive.begin( ), last;
                    plf::advance( first, dist1 );
                    last = first;
                    plf::advance( last, dist2 );

                    int const dist = static_cast<int>( plf::distance( first, last ) );

                    if ( dist != dist2 )
                    {
                        printf(
                            "positive distance overload fuzz-test failed, real distance = %d, reported distance = %d, counter = %d, suite loop = %d",
                            dist2, dist, counter, looper );
                        getchar( );
                        abort( );
                    }
                }
                ...
                for ( int counter = 0; counter != 10000; ++counter )
                {
                    ...
                    auto first = d_hive.begin( ), last           = d_hive.begin( );
                    if ( dist1 > dist2 ) { std::swap( dist1, dist2 ); }
                    plf::advance( first, dist1 );
                    plf::advance( last, dist2 );

                    if ( int const dist = static_cast<int>( plf::distance( first, last ) ); dist != dist2 - dist1 )
                    {
                        printf(
                            "positive/negative distance overload fuzz-test failed, real distance = %d, reported distance = %d, counter = %d, suite loop = %d",
                            dist2, dist, counter, looper );
                        getchar( );
                        abort( );
                    }
            ...
            for ( auto the_iterator = hive_copy.begin( ); the_iterator != hive_copy.end( ); ++the_iterator )
            {
                ++total;
            }
            ...
            auto next_iterator = plf::next( p_hive.begin( ), 5 );
            auto prev_iterator = plf::prev( p_hive.cend( ), 300 );

            failpass( "Iterator next test", plf::distance( p_hive.begin( ), next_iterator ) == 5 );
            failpass( "Const iterator prev test", plf::distance( prev_iterator, p_hive.cend( ) ) == 300 );

            auto prev_iterator2 = plf::prev( p_hive.end( ), 300 );
            failpass( "Iterator/Const iterator equality operator test", prev_iterator == prev_iterator2 );

            prev_iterator = p_hive.begin( );
            plf::advance( prev_iterator, 5 );
            ...
            for ( auto the_iterator = p_hive.rbegin( ); the_iterator != p_hive.rend( ); ++the_iterator )
            {
                ++total;
                numtotal += **the_iterator;
            }
            ...
            auto r_iterator = p_hive.rbegin( );
            plf::advance( r_iterator, 50 );

            failpass( "Reverse iterator advance and distance test", plf::distance( p_hive.rbegin( ), r_iterator ) == 50 );

            auto r_iterator2 = plf::next( r_iterator, 2 );

            failpass( "Reverse iterator next and distance test", plf::distance( p_hive.rbegin( ), r_iterator2 ) == 52 );


            hive<int*>::reverse_iterator r_iterator3 = std::make_reverse_iterator( ++( p_hive.begin( ) ) );

            failpass( "std::reverse_iterator and negative distance test", plf::distance( r_iterator3, p_hive.rend( ) ) == 1 );


            numtotal = 0;
            total    = 0;

            for ( auto the_iterator = p_hive.begin( ); the_iterator < p_hive.end( ); plf::advance( the_iterator, 2 ) )
            {
                ++total;
                numtotal += **the_iterator;
            }
            ...
            for ( auto the_iterator = p_hive.cbegin( ); the_iterator != p_hive.cend( ); ++the_iterator )
            {
                ++total;
                numtotal += **the_iterator;
            }
            ...
            for ( auto the_iterator = --hive<int*>::const_reverse_iterator( p_hive.crend( ) );
                  the_iterator != hive<int*>::const_reverse_iterator( p_hive.crbegin( ) ); --the_iterator )
            {
                ++total;
                numtotal += **the_iterator;
            }
            ...
            for ( auto the_iterator = ++hive<int*>::iterator( p_hive.begin( ) ); the_iterator < p_hive.end( ); ++the_iterator )
            {
                ++total;
            ...
            unsigned int const temp_capacity = static_cast<unsigned int>( p_hive.capacity( ) );
            ...
            for ( auto the_iterator = --hive<int*>::iterator( p_hive.end( ) ); the_iterator != p_hive.begin( ); --the_iterator )
            {
                ++total;
            }
            ...
            for ( auto the_iterator = --( hive<int*>::iterator( p_hive.end( ) ) ); the_iterator != p_hive.begin( ); plf::advance(
                      the_iterator, -2 ) )
            {
                ++total;
            }
            ...
            auto it1 = i_hive.begin( ), it2 = i_hive.begin( );
            ...
            auto found_item = std::find( i_hive.begin( ), i_hive.end( ), 5000 );;

            failpass( "std::find iterator test", *found_item == 5000 );


            auto found_item2 = std::find( i_hive.rbegin( ), i_hive.rend( ), 5000 );;

            failpass( "std::find reverse_iterator test", *found_item2 == 5000 );


            for ( auto the_iterator = i_hive.begin( ); the_iterator != i_hive.end( ); ++the_iterator )
            {
            ...
            do
            {
                for ( auto the_iterator = i_hive.begin( ); the_iterator != i_hive.end( ); )
                {
                    if ( ( rand( ) & 7 ) == 0 )
                    {
                        the_iterator = i_hive.erase( the_iterator );
            ...
            auto it2 = i_hive.begin( );
            plf::advance( it2, 250000 );
            ...
            auto end_iterator  = i_hive.end( );
            auto end_iterator2 = i_hive.end( );
            plf::advance( end_iterator, -250000 );
            ...
            for ( auto the_iterator = i_hive.begin( ); the_iterator != end_iterator; )
            {
                the_iterator = i_hive.erase( the_iterator );
            }
            ...
            plf::advance( end_iterator, -50001 );
            auto begin_iterator = i_hive.begin( );
            plf::advance( begin_iterator, 300000 );

            for ( auto the_iterator = begin_iterator; the_iterator != end_iterator; )
            {
                the_iterator = i_hive.erase( the_iterator );
            }
            ...
            plf::advance( begin_iterator, 300001 );


            for ( auto the_iterator = begin_iterator; the_iterator != i_hive.end( ); )
            {
                the_iterator = i_hive.erase( the_iterator );
            }
            ...
            auto temp_iterator = i_hive.begin( );
            plf::advance( temp_iterator, 20 ); // Advance test 1

            unsigned int index = static_cast<unsigned int>( plf::distance( i_hive.begin( ), temp_iterator ) );
            ...
            plf::advance( temp_iterator, 500 );

            index = static_cast<unsigned int>( plf::distance( i_hive.begin( ), temp_iterator ) );

            failpass( "Advance + iterator-to-index test", index == 500 );

            auto temp2 = i_hive.get_iterator( &( *temp_iterator ) );
            auto temp3 = i_hive.get_iterator( const_cast<hive<int>::const_pointer>( &( *temp_iterator ) ) );

            failpass( "Pointer-to-iterator test", temp2 != i_hive.end( ) );
            failpass( "Const_pointer-to-const_iterator test", temp3 != i_hive.end( ) );

            temp2 = i_hive.begin( );
            plf::advance( temp2, 500 );
            ...
            unsigned int const temp_capacity2 = static_cast<unsigned int>( i_hive.capacity( ) );
            ...
            plf::advance( it1, 400 );
            plf::advance( it2, 500 ); // This should put it2 past the point of previous erasures
            ...
            plf::advance( it1, 4 );
            plf::advance( it2, 9 ); // This should put it2 past the point of previous erasures
            ...
            plf::advance( it1, 345 ); // Test erasing and validity when it removes the final group in hive
            i_hive.erase( it1, it2 );
            ...
            failpass(
                "Range-erase with hive already half-erased, alternating erasures", counter == 904 && i_hive.size( ) == 904 );
            ...
            failpass(
                "Range-erase with hive already third-erased, randomized erasures", counter == 400 && i_hive.size( ) == 400 );
            ...
            for ( unsigned int loop_counter = 0; loop_counter != 50; ++loop_counter )
            {
                ...
                while ( !i_hive.empty( ) )
                {
                    ...
                    plf::advance( it1, static_cast<int>( range1 ) );
                    plf::advance( it2, static_cast<int>( range2 ) );
                    ...
                    {
                        printf(
                            "Fuzz-test range-erase randomly Fail: loop counter: %u, internal_loop_counter: %u.\n", loop_counter,
                            internal_loop_counter );
                        getchar( );
                        abort( );
                    }
            ...
            for ( unsigned int loop_counter = 0; loop_counter != 50; ++loop_counter )
            {
                ...
                while ( !i_hive.empty( ) )
                {
                    ...
                    {
                        printf(
                            "Fuzz-test range-erase + fill-insert randomly Fails during erase: loop counter: %u, internal_loop_counter: %u.\n",
                    ...
                    if ( i_hive.size( ) > 100 )
                    {
                        ...
                        unsigned int const extra_size = rand( ) & 127;
                        i_hive.insert( extra_size, 5 );

                        if ( i_hive.size( ) != static_cast<unsigned int>( counter ) + extra_size )
                        {
                            printf(
                                "Fuzz-test range-erase + fill-insert randomly Fails during fill-insert: loop counter: %u, internal_loop_counter: %u.\n",
                                loop_counter, internal_loop_counter );
                            getchar( );
                            abort( );
                        }
                        ...
                        {
                            printf(
                                "Fuzz-test range-erase + fill-insert randomly Fails during counter-test fill-insert: loop counter: %u, internal_loop_counter: %u.\n",
                                loop_counter, internal_loop_counter );
                            getchar( );
                            abort( );
                        }
            ...
            for ( auto ss_it = ss_nt.begin( ); ss_it != ss_nt.end( ); ++ss_it )
            {
            ...
            for ( unsigned int loop_counter = 0; loop_counter != 50; ++loop_counter )
            {
                ...
                while ( !ss_nt.empty( ) )
                {
                    ...
                    range1 = rand( ) % size;
                    range2 = range1 + 1 + ( rand( ) % ( size - range1 ) );
                    plf::advance( ss_it1, static_cast<int>( range1 ) );
                    plf::advance( ss_it2, static_cast<int>( range2 ) );
            ...
            for ( auto current = i_hive.begin( ); current != i_hive.end( ); ++current )
            {
                if ( previous > *current )
                {
                    sorted = false;
            ...
            for ( auto current = ss_hive.begin( ); current != ss_hive.end( ); ++current )
            {
            ...
            for ( auto it = i_hive2.begin( ); it != i_hive2.end( ); ++it )
            {
                total += *it;
            }
            ...
            {
                unsigned int const capacity = rand( ) & 65535;
                ...
                {
                    printf(
                        "Fuzz-test assign capacity Fail: global loop counter: %u, internal loop counter: %u.\n", looper,
                        internal_loop_counter );
                    getchar( );
                    abort( );
                }
                ...
                {
                    printf(
                        "Fuzz-test assign sum Fail: global loop counter: %u, internal loop counter: %u.\n", looper,
                        internal_loop_counter );
                    getchar( );
                    abort( );
                }
            ...
            for ( unsigned int internal_loop_counter = 0; internal_loop_counter != 10; ++internal_loop_counter )
            {
                ...
                for ( auto it3 = i_hive.begin( ); it3 != i_hive.end( ); ++it3 )
                {
                    total += *it3;
                }
                ...
                {
                    printf(
                        "Fuzz-test range assign capacity Fail: global loop counter: %u, internal loop counter: %u.\n", looper,
                        internal_loop_counter );
                    getchar( );
                    abort( );
                }
                ...
                {
                    printf(
                        "Fuzz-test range assign sum Fail: global loop counter: %u, internal loop counter: %u.\n", looper,
                        internal_loop_counter );
                    getchar( );
                    abort( );
                }
            ...
            for ( unsigned int internal_loop_counter = 0; internal_loop_counter != 10; ++internal_loop_counter )
            {
                ...
                for ( auto it3 = i_hive.begin( ); it3 != i_hive.end( ); )
                {
                    if ( ( rand( ) & 7 ) == 0 )
                    {
                ...
                {
                    printf(
                        "Fuzz-test range assign + erase capacity Fail: global loop counter: %u, internal loop counter: %u.\n",
                ...
                {
                    printf(
                        "Fuzz-test range assign + erase sum Fail: global loop counter: %u, internal loop counter: %u.\n", looper,
                        internal_loop_counter );
                    getchar( );
                    abort( );
                }
            ...
            for ( auto it = ss_hive.begin( ); it != ss_hive.end( ); ++it )
            {
                total2 += it->number;
            }
            ...
            title2( "Splice tests" ); {
                ...
                for ( auto current = hive1.begin( ); current != hive1.end( ); ++current )
                {
                ...
                for ( auto current = hive2.begin( ); current != hive2.end( ); )
                {
                    if ( ( rand( ) & 7 ) == 0 )
                    {
                ...
                for ( auto current = hive1.begin( ); current != hive1.end( ); )
                {
                    if ( ( rand( ) & 1 ) == 0 )
                    {
                ...
                unsigned int const total = static_cast<unsigned int>( hive1.size( ) + hive2.size( ) );
            ...
            int const count0 = static_cast<int>( std::count( i_hive.begin( ), i_hive.end( ), 0 ) );
            int const count1 = 1000 - count0;
